'''Данный код реализует класс Deque для работы с двусторонней очередью (деком). 
В классе есть методы для добавления элементов в начало и конец дека (push_front, push_back), 
а также для удаления элементов с начала и конца дека (pop_front, pop_back).
Класс также определяет два исключения: DequeFullError - возникает, если попытаться добавить элемент в полный дек, 
и EmptyDequeError - возникает, если попытаться удалить элемент из пустого дека.
В основной части кода пользователь может ввести количество операций и максимальный размер дека. 
Затем пользователь может вводить команды для работы с деком, где первый элемент в строке - это команда, 
а если есть второй элемент - это значение для добавления в дек.'''
